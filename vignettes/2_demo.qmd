...
For the sake of this demonstration, we will use point distribution data which are generated from a *Kent distribution* (the spherical equivalent of a bivariate Gaussian distribution). The `orange` package has a bunch of samples generated from multiple such distributions for testing and similar demonstration purposes, which can be accessed with the `kentssamples` built-in data.
