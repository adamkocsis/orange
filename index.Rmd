...
- `coords`: A 2 column tabular structure that inherits either from `matrix` or `data.frame`
- `sfpoints`: A multipoint object, either of classes `sf` or `sfc`. 
- `icell`: Vector of cell identifiers of an icosahedral grid (`icosa::hexagrid` or `icosa::trigrid`), a `character` vector. Note that these can only be used together with an `igrid` type icosahedral grid. Every possible method that can be applied to points (`coords` or `sfpoints`) can also be calculated for these by substituting the points with the coordinates of the grid cell center, frequently leading to different estimation properties.
