...
# get an operative grid
suppressMessages(hex <- hexagrid(deg=1, sf=TRUE))
...
occ <- unique(locate(hex, mat))
manual <- gappiness(occ, hex)


# simple matrix solution
expect_silent(gpMat <- gappiness(mat, s=hex))
expect_equal(manual, gpMat)

# with full outpu
expect_silent(gpMatFull <- gappiness(mat, s=hex, full=TRUE))
expect_equal(gappiness(gpMatFull$occupied, hex), gpMatFull$estimate)
expect_equal(gpMat, gpMatFull$estimate)

# duplicates make no difference
expect_silent(gpMatDupl <- gappiness(mat, s=hex, duplicates=TRUE))
expect_equal(gpMatDupl, gpMat)
